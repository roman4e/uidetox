...
```ts
import { state, effect } from 'ui-detox';
...
```ts
import { scheduleRead, readFrame } from 'ui-detox';
...
```ts
import { mutate, mutateStructural, readStaged, measure } from 'ui-detox';
...
```ts
import { flip, animate, viewTransition } from 'ui-detox';
...
| `readFrame(fn)` → `Promise<T>` | run `fn` after the next DOM commit (post-layout reads) — also importable from `ui-detox` |
| `task(fn, opts?)` | instance-scoped task, auto-disposed (see §10) |
| `onCleanup(fn)` | teardown run on disconnect (also importable from `ui-detox`) |
...
**`onCleanup(fn)`** (from `ui-detox`) registers unmount teardown from a free
...
explicit `import state from "ui-detox"` still works and is de-duplicated.) The
...
- A specifier containing `/` (`"ui-detox/forms"`, `"./x.dtx"`) is an npm/explicit
...
import form, f from "ui-detox/forms"       → import { form, f } from "ui-detox/forms"
```
...
```ts
import { state, task } from 'ui-detox';
const s = state({ q: '' });
...
```ts
import { defineEmits } from 'ui-detox';
defineComponent({
  tag: 'rating-stars',
  boot: (ctx) => {
...
```ts
import { createToken, registry } from 'ui-detox';
...
```ts
import { defineRouter, registerOutlet, lazy, Redirect } from 'ui-detox';
...
```ts
import { defineTrait } from 'ui-detox';
defineTrait('trim', {
...
```ts
import { defineBehaviorTrait } from 'ui-detox';
...
```ts
import { defineFilter } from 'ui-detox';
...
```ts
import { createHttpClient, resource, mutation, ApiError } from 'ui-detox/http';
```
...
```ts
import { renderToString, hydrate } from 'ui-detox';
```
...
```ts
import { inspectComponentTree } from 'ui-detox';
...
| Import | Provides |
|---|---|
| `ui-detox` | runtime: reactivity, `defineComponent`, directives helpers, registry, router, traits, filters, dnd, anim, DOM staging, SSR, devtools |
| `ui-detox/forms` | `f`, `form`, `registerFormComponents` |
| `ui-detox/http` | `createHttpClient`, `resource`, `mutation`, `ApiError` |
