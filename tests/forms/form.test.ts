...
describe('form()', () => {
  ...
  it('field setValue updates values, clears its error, sets dirty', () => {
    const fm = make();
    fm.field('name').setValue('Salt');
    expect(fm.values.name).toBe('Salt');
    expect(fm.errors['name']).toBeUndefined();
    expect(fm.field('name').dirty).toBe(true);
  ...
  it('cross-field rule', () => {
    ...
    expect(fm.errors['neto']).toContain('neto must not exceed brutto');
